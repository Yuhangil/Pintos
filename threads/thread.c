...
void
update_next_tick_to_wake(int64_t ticks)
{
    next_tick_to_wake = (next_tick_to_wake > ticks) ? ticks : next_tick_to_wake;
}
...
thread_wake(int64_t ticks)
{
    ...
    struct thread * t;
    struct list_elem *n;
    n = list_begin(&block_list);
    while(n != list_end(&block_list))
    {
        t = list_entry(n, struct thread, elem);
	if(ticks >= t->wake_up_tick)
	{
	    n = list_remove(&t->elem);
	    thread_unblock(t);
	}
	else
	{
	    n = list_next(n);
	    update_next_tick_to_wake(t->wake_up_tick);
	}
    }
}
